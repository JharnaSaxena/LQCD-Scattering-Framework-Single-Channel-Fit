...
def get_threshold_energies(L, mass_pion=1.0):
    """
    Compute threshold energies for different channels.
    For πΣ, ππΛ, and K̄N thresholds.
    """
    two_pi_over_L = 2.0 * np.pi / L
...
def plot_figure8_from_hdf5(hdf5_path, L, use_ref=True, energy_window=(6.5, 8.5), save_path=None):
    """
    Generate Figure 8 directly from HDF5 file with beautiful styling.
    """
    ...
    # Also add ππ threshold if it's in range
    ...
    # ── Layer 1: Gray bands (behind everything) ─────────────────────────────
    band_width = 0.5
    ...
    # ── Layer 2: Threshold lines (dashed, across entire plot) ──────────────
    xmin, xmax = -0.6, n_cat - 0.4
    
    # Color thresholds differently
    threshold_colors = {
        r'$\pi\Sigma$': '#1a1a1a',
        r'$\bar{K}N$': '#2c3e50',
    ...
    # ── Layer 3: Data points (green with error bars) ────────────────────────
    for i, cat in enumerate(categories):
        x0 = x_positions[i]
        pts = points.get(cat, [])
        n_pts = len(pts)
    ...
    # ── Layer 4: Level number annotations ───────────────────────────────────
    for i, cat in enumerate(categories):
        x0 = x_positions[i]
        pts = points.get(cat, [])
    ...
    # Spines with ticks on all sides
