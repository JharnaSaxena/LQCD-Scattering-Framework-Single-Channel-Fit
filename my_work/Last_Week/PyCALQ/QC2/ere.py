...
(k/mπ) cotδ = (E/mπ) [a + bdelta]
where
delta = (E^2- threshold^2) / threshold^2

The implementation supports both the energy-based form used in the paper and a simpler q²-based expansion for testing
"""
import numpy as np
...
class ERE:
    def __init__(self, coeffs: Union[List[float], np.ndarray]):
        #coeffs: [a, b] for the ERE parametrization
        #Delta = (E^2 - threshold^2) / threshold^2  paper Eq.12
    ...
    def compute_kinv(self, kin: Union[KinematicVarsLike, float]) -> float:
        ...
        if hasattr(kin, "E_cm") and hasattr(kin, "threshold"):
            ...
            # A small offset is included to avoid division by zero
            # if the threshold is numerically zero.
            Delta = (E*E - th*th) / (th*th + 1e-15)
            # Evaluate the ERE expression:
            # k cot(delta) = E(a + b delta)
